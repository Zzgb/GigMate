 ...
 return (
  <div className="flex flex-col flex-1">
   ...
   <main className="flex-1 p-8 max-w-5xl mx-auto w-full">
    <a href="/dashboard/my-tasks" className="text-sm text-[var(--g-text2)] hover:text-[var(--g-text)]">← 返回我的任务</a>
    <div className="flex justify-between items-center mb-6 mt-4">
     <div>
      <h4 className="text-base font-semibold mb-0.5">{task?.title || "任务"} · 申请列表</h4>
      <div className="text-xs text-[var(--g-text2)] dark:text-[#98989d]">共 {filtered.length} 人申请 | {task?.status === "OPEN" ? "招募中" : task?.status === "IN_PROGRESS" ? "进行中" : "已完成"}</div>
     </div>
     <div className="flex gap-1.5">
      {(["全部", "待审核", "已通过"] as const).map((f) => (
       <button
        key={f}
        onClick={() => setFilter(f)}
        className={`px-3.5 py-1 rounded-full text-xs cursor-pointer ${filter === f ? "bg-black text-white" : "bg-[var(--g-card)] text-[var(--g-text2)] border border-[var(--g-border3)]"}`}
       >
        {f}
       </button>
      ))}
    ...
    <div className="flex flex-col gap-3">
     {filtered.map((a: any) => (
      <div key={a.id} className="bg-[var(--g-card)] rounded-2xl p-5 border border-[var(--g-border)] shadow-[0_4px_24px_var(--g-shadow)]">
       <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
         <div className="flex items-center gap-2 mb-0.5">
          ...
          <span className="text-xs text-[var(--g-text2)] dark:text-[#98989d]">申请中</span>
         </div>
         <p className="text-xs text-[var(--g-text2)] line-clamp-2">{a.message}</p>
        </div>
     ...
     {filtered.length === 0 && (
      <div className="text-center py-10 text-sm text-[var(--g-text2)] dark:text-[#98989d]">没有匹配的申请</div>
     )}
    </div>
   </main>
  </div>
