      ...
      <main className="flex-1 p-8 max-w-5xl mx-auto w-full">
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="w-[240px] flex-shrink-0 flex flex-col gap-3">
            <div className="bg-white rounded-[16px] p-4 shadow-[0_2px_20px_rgba(0,0,0,0.04)]">
              ...
              <div className="flex gap-1.5 items-center">
                <input type="text" value={form.budgetMin} onChange={(e) => update("budgetMin", e.target.value)} placeholder="最低"
                  className="w-full bg-[#f5f5f7] rounded-lg px-2 py-2 text-xs outline-none" />
            ...
            <div className="bg-white rounded-[16px] p-4 shadow-[0_2px_20px_rgba(0,0,0,0.04)]">
              ...
              <input type="text" value={form.category} onChange={(e) => update("category", e.target.value)}
                placeholder="如 设计、翻译、技术" className="w-full bg-[#f5f5f7] rounded-lg px-3 py-2 text-xs outline-none" />
            </div>
            <div className="bg-white rounded-[16px] p-4 shadow-[0_2px_20px_rgba(0,0,0,0.04)]">
              ...
              <input type="text" value={form.duration} onChange={(e) => update("duration", e.target.value)}
                placeholder="如 2 周" className="w-full bg-[#f5f5f7] rounded-lg px-3 py-2 text-xs outline-none" />
              ...
              <input type="text" value={form.workMode} onChange={(e) => update("workMode", e.target.value)}
                placeholder="远程/线下/混合" className="w-full bg-[#f5f5f7] rounded-lg px-3 py-2 text-xs outline-none" />
              <div className="text-xs font-medium mb-2 mt-3">工作地点</div>
              <input type="text" value={form.location} onChange={(e) => update("location", e.target.value)}
                placeholder="城市或具体地址" className="w-full bg-[#f5f5f7] rounded-lg px-3 py-2 text-xs outline-none" />
              <div className="text-xs font-medium mb-2 mt-3">截止日期</div>
              <input type="date" value={form.deadline} onChange={(e) => update("deadline", e.target.value)}
                className="w-full bg-[#f5f5f7] rounded-lg px-3 py-2 text-xs outline-none" />
            </div>
