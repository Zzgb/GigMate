 ...
 return (
  <div>
   ...
   <div className="flex flex-col gap-3">
    {workers.map((w) => (
     <div key={w.name} className="bg-[var(--g-card)] rounded-2xl border border-[var(--g-border)] shadow-[0_4px_24px_var(--g-shadow)] overflow-hidden">
      {/* 主行 */}
      <div className="p-5 flex items-center gap-4">
       ...
       <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
         ...
         <span className="text-xs text-[var(--g-text2)] dark:text-[#98989d]">{w.task}</span>
        </div>
        <div className="flex items-center gap-2">
         <span className="text-xs bg-[#007aff1a] text-[#007aff] px-2 py-0.5 rounded-full font-medium">{w.status}</span>
         <span className="text-xs text-[var(--g-text2)] dark:text-[#98989d]">通过: {w.startedAt}</span>
         <span className="text-xs text-[var(--g-text2)] dark:text-[#98989d]">截止: {w.deadline}</span>
        </div>
       </div>
       <button
       ...
       <div className="border-t border-[var(--g-border2)] bg-[var(--g-input)]">
        ...
        <div className="px-5 py-2.5 bg-[var(--g-card)] border-b border-[var(--g-border)]">
         <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-[var(--g-text2)] ml-auto">通过: {w.startedAt} · 截止: {w.deadline}</span>
         </div>
        </div>
        {/* 消息列表 */}
