    ...
    <div className="flex-1">
     <div className="flex justify-between items-start mb-2">
      <div>
       ...
       <span className="text-xs text-[var(--g-text2)] ml-3">{experience}</span>
      </div>
      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusStyle}`}>{statusLabel}</span>
     </div>
     <p className="text-xs text-[var(--g-text2)] mb-2 leading-relaxed">{message}</p>
     <div className="flex gap-1.5 mb-2">
      {tags.map((t) => (
       <span key={t} className="text-[10px] bg-[var(--g-input)] px-1.5 py-1 rounded-md text-[var(--g-text2)] dark:text-[#98989d]">{t}</span>
      ))}
     </div>
     <div className="flex justify-between items-center">
      <div className="flex gap-3">
       <span className="text-[10px] text-[var(--g-text2)] dark:text-[#98989d]">⭐ {rating}</span>
       <span className="text-[10px] text-[var(--g-text2)] dark:text-[#98989d]">已完成 {completed} 单</span>
       <span className="text-[10px] text-[var(--g-text2)] dark:text-[#98989d]">回复率 {responseRate}</span>
      </div>
      {status === "pending" ? (
       <div className="flex gap-2">
        ...
        <button onClick={() => {}} className="bg-[var(--g-card)] px-3.5 py-1.5 rounded-full text-xs text-[var(--g-text2)] border border-[rgba(0,0,0,0.1)] cursor-pointer">拒绝</button>
       </div>
      ) : (
