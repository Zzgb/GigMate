          ...
          <div key={item.key} className="relative">
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm cursor-pointer ${
                active ? "bg-black text-white" : "bg-white text-[#86868b] dark:text-[#98989d] border border-[rgba(0,0,0,0.06)] dark:border-[rgba(255,255,255,0.08)]"
              }`}
            >
              {item.key}
              ...
              <div className="absolute top-full left-0 mt-1.5 bg-white rounded-xl shadow-[0_8px_40px_rgba(0,0,0,0.12)] min-w-[150px] py-1 z-50">
                {item.options!.map((opt) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-xs text-[#1d1d1f] dark:text-white hover:bg-[#f5f5f7] cursor-pointer"
                  >
                    {opt}
                  </button>
                ))}
