     ...
     <div key={item.key} className="relative">
      <button
       ...
       className={`px-4 py-1.5 rounded-full text-sm cursor-pointer ${
        active ? "bg-black text-white" : "bg-[var(--g-card)] text-[var(--g-text2)] border border-[var(--g-border3)]"
       }`}
      >
       {item.key}
      ...
      {item.type === "dropdown" && openFilter === item.key && (
       <div className="absolute top-full left-0 mt-1.5 bg-[var(--g-card)] rounded-xl shadow-[0_8px_40px_var(--g-shadow-lg)] min-w-[150px] py-1 z-50">
        {item.options!.map((opt) => (
         <button
          key={opt}
          onClick={() => handleOptionSelect(item.key, opt)}
          className="w-full text-left px-4 py-2 text-xs text-[var(--g-text)] hover:bg-[var(--g-input)] cursor-pointer"
         >
          {opt}
         </button>
        ))}
