...
export default function Nav({ variant = "landing" }: NavProps) {
  ...
  const linkClass = (path: string) => {
    ...
    return isActive ? "font-semibold text-[#1d1d1f] dark:text-white dark:text-white" : "text-[#86868b] dark:text-[#98989d] hover:text-[#1d1d1f] dark:text-white dark:text-white";
  };
  ...
  return (
    <nav className="flex justify-between items-center px-6 py-3 bg-white/80 dark:bg-[#2c2c2e]/80 backdrop-blur-xl border-b border-[rgba(0,0,0,0.06)] dark:border-[rgba(255,255,255,0.08)] relative z-30">
      <div className="flex items-center gap-8">
        ...
        {variant === "dashboard" || isLoggedIn ? navLinks : null}
      </div>
      <div className="flex items-center gap-4">
        {variant === "landing" && !isLoggedIn ? (
          <>
            <a href="/login" className="text-sm text-[#1d1d1f] dark:text-white dark:text-white">登录</a>
          ...
          <>
            ...
            <span className="text-xs text-[#86868b] dark:text-[#98989d] dark:text-[#98989d]">{name}</span>
            <AvatarMenu />
          </>
        ) : null}
      </div>
