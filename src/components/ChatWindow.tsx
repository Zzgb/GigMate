/**
 * ChatWindow.tsx
 * 聊天窗口 - 消息显示 + 里程碑审批卡片（融入消息流）+ 验收提交入口
 * 修改日期: 2026-05-25
 */
...
interface ChatWindowProps {
  ...
  // 里程碑
  approvalMap?: Record<string, any>;
  isEmployer?: boolean;
  isFreelancer?: boolean;
  taskStatus?: string;
  hasSubmittableMilestones?: boolean;
  onApproveMilestone?: (approvalId: string) => Promise<void>;
  onRejectMilestone?: (approvalId: string, reason: string) => Promise<void>;
  onOpenMilestoneForm?: () => void;
}

export default function ChatWindow({
  name, task, taskId, avatarUrl, myAvatarUrl, messages, onSend,
  approvalMap, isEmployer, isFreelancer, taskStatus, hasSubmittableMilestones,
  onApproveMilestone, onRejectMilestone, onOpenMilestoneForm,
}: ChatWindowProps) {
  ...
  useEffect(() => {
    if (!approvalMap) return;
    const count = Object.keys(approvalMap).length;
    if (count > 0 && count !== approvalCountRef.current) {
      approvalCountRef.current = count;
      scrollToBottom();
    }
  }, [approvalMap]);
  ...
  const handleSend = async () => {
    ...
    try { if (onSend) { await onSend(input); } setInput(""); }
  ...
  return (
    <div className="flex-1 flex flex-col bg-[var(--g-card)] min-w-0 min-h-0">
              ...
              <div>
                <div className={`rounded-2xl px-4 py-2.5 max-w-[400px] ${m.from === "other" ? "bg-[var(--g-input)] rounded-bl-md" : "bg-[#1d1d1f] rounded-br-md text-white"}`}>
                  {m.text.startsWith("[文件] ") ? (() => {
                    ...
                    return (
                      <div>
                        {isImage ? (<a href={url} target="_blank" rel="noopener noreferrer"><img src={url} alt={name} className="max-w-[200px] max-h-[150px] rounded-lg object-cover mb-1" /></a>)
                          : (<div className="mb-1"><span className="text-[10px] truncate max-w-[160px]">{name}</span></div>)}
      ...
      <div className="px-6 py-3 border-t border-[var(--g-border2)] flex gap-2 items-center">
        ...
        {isFreelancer && taskStatus === "IN_PROGRESS" && hasSubmittableMilestones && onOpenMilestoneForm && (
          <button onClick={onOpenMilestoneForm}
            className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-medium bg-[#007aff1a] text-[#007aff] hover:bg-[#007aff22] cursor-pointer">
            <ClipboardCheck className="w-3 h-3" />提交验收
          </button>
        )}
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && handleSend()}
          placeholder="输入消息..." className="flex-1 bg-[var(--g-input)] rounded-xl px-4 py-2 text-xs outline-none" />
        <button onClick={handleSend}
          className="bg-black text-white px-5 py-2 rounded-xl text-xs font-medium cursor-pointer" type="button">发送</button>
      </div>
    </div>
  );
}
